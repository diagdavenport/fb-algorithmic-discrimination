...
l.dat$cat <- factor(l.dat$cat,levels = c("intention", "access", "awareness", "control", "influence", "efficiency", "cogeffort", "gut", "careful", "simple composite"))
 ...
 ggplot(l.dat %>% filter(cat != "careful" & cat != "gut"), aes(x = cat, y = mean.val, fill = target)) +
  geom_col(position = "dodge", aes(fill = target)) +
   ...
   scale_fill_manual(values=c("#39568CFF", "#FDE725FF"),
                       name="Target",
                       breaks=c("nf", "pymk"),
...
cohen.data$ci <- with(cohen.data, (sd/100)*1.96)
pd <- position_dodge(0.01)

 ggplot(cohen.data, aes(x = cat, y = abs(cohen.d))) +
   geom_errorbar(aes(ymin=abs(cohen.d)-ci, ymax=abs(cohen.d)+ci), colour="black", width=.1, position=pd) + ylim(c(-.6, .6)) +
  geom_point(position = "dodge") + geom_hline(yintercept = c(.2,.5,-.2, -.5), linetype = "dotted") + coord_flip()
 
  ggplot(cohen.data, aes(x = cat, y = abs(cohen.d))) +
  geom_point(position = "dodge") + geom_hline(yintercept = c(.2,.5,-.2, -.5), linetype = "dotted") + ylim(c(-.6, .6)) +
...
ggplot(aes(x=x_new)) + stat_ecdf(aes(colour=target)) +
   scale_color_manual(values=c("#39568CFF", "#FDE725FF"))

...
```{r helper vars for pymk and graphing}

m.newsfeed.df.all$group <- with(m.newsfeed.df.all, ifelse(race.in.group == T, "Ingroup", "Outgroup"))
m.newsfeed.df.all$group <- factor(m.newsfeed.df.all$group, levels = c("Outgroup", "Ingroup"))
...
```{r drop single post participants}

nf.counts <- m.newsfeed.df.all %>% group_by(primary.id) %>% dplyr::summarise(count = n())
drop.ids <- (nf.counts %>% filter(count == 1))$primary.id
...
m.all <- m.newsfeed.df.all %>% 
  ...
  dplyr::select(dedupid, preference, run_count) %>%
...
temp.m <- newsfeed.with.misranks %>%
  ...
  mutate(would.ord = row_number()) %>% dplyr::select(preference, dedupid, would.ord) %>%
...
newsfeed.df.w.logins <- merge(newsfeed.with.misranks,
              m.qual.df.all %>% dplyr::select(last.login, primary.id),
              by = "primary.id",
              all.x = T,
              all.y = F)
```
...
ggplot(l.dat, aes(x=norm.quartile, fill = race.in.group, y=rate)) + 
  ...
  scale_fill_manual(values=outgroup.ingroup.colors, 
                       ...
                       breaks=c("FALSE", "TRUE"),
                       labels=c("Outgroup", "Ingroup")) +
  xlab("User self-reported preference quartile")
...
m.newsfeed.df.all$newquartile4 <- m.newsfeed.df.all$newquartile4 + 1
m.newsfeed.df.all$new.order4 <- m.newsfeed.df.all$new.order4 + 1
m.newsfeed.df.all$chron.order4 <- m.newsfeed.df.all$chron.order4 + 1
...
ggplot(as.data.frame(l.dat),
  ...
  scale_x_discrete(limits = c("Self-Reported Preference Quartile", "Actual Newsfeed Ranking Quartile"), expand = c(.05, .05), position = "top") + theme(legend.position="bottom") + 
...
ggplot(as.data.frame(l.dat),
  ...
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Chronological Ranking Quartile", "Actual Newsfeed Ranking Quartile"), expand = c(.05, .05), position = "top") + 
  scale_fill_manual(values=rev(brewer.pal(n = 5, name = "Blues")[2:5])) + 
  theme(axis.title.y =element_blank(),
        axis.text.y =element_blank(),
        axis.ticks.y = element_blank(), legend.position="none")
...
m.pymk.df.all$new.order4 <- ifelse(m.pymk.df.all$new.order4 == 4, 3, m.pymk.df.all$new.order4) + 1
m.pymk.df.all$friend.order4 <- trunc(m.pymk.df.all$pct_friend_rank/15)
m.pymk.df.all$friend.order4 <- ifelse(m.pymk.df.all$friend.order4 == 4, 3, m.pymk.df.all$friend.order4) + 1

m.pymk.df.all$newquartile4 <- 3 - (m.pymk.df.all$norm.quartile - 1) + 1
...
ggplot(as.data.frame(l.dat),
  ...
  geom_stratum(width = 1/12, fill = "white", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Ranking Group*(-1)", "Actual PYMK Ranking"), expand = c(.05, .05), position = "top")
  scale_fill_manual(values=rev(brewer.pal(n = 5, name = "Blues")[2:5])) + 
  theme(axis.title.y =element_blank(),
        axis.text.y =element_blank(),
        axis.ticks.y = element_blank(), legend.position="none")
...
ggplot(as.data.frame(l.dat),
  ...
  geom_stratum(width = 1/12, fill = "white", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Pct Friend Group", "Actual PYMK Ranking"), expand = c(.05, .05), position = "top") +
  scale_fill_manual(values=rev(brewer.pal(n = 5, name = "Blues")[2:5])) + 
  theme(axis.title.y =element_blank(),
        axis.text.y =element_blank(),
        axis.ticks.y = element_blank(), legend.position="none")
...
heatmap.label.nf.order <- "Actual Newsfeed Ranking"
heatmap.label.complete_time_rank <- "Chronological Ranking"
heatmap.label.norm_pctle <- "User's Self-Reported Preferences"

heat.df <- m.newsfeed.df.all %>%
...
ggplot(m.pymk.df.all, aes(x=pct_friend_rank, pymk.order)) + stat_density_2d(
  geom = "raster",
...
ggplot(m.newsfeed.df.all, aes(x=complete_time_rank, nf.order)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)),
  contour = FALSE
) + scale_x_reverse() + scale_y_reverse()  + scale_fill_gradientn(colors = brewer.pal(n = 5, name = "Blues"), values = color.scaling) + xlab(heatmap.label.complete_time_rank) + ylab(heatmap.label.nf.order)

dev.copy(jpeg,'Output/Graphs/Audit/Heatmaps/US NF chron rank by nf rank - smooth.jpg')
dev.off()

ratio.by.combo <- m.newsfeed.df.all %>%
...
ggplot(m.newsfeed.df.all, aes(x=norm.pctle, nf.order)) + stat_density_2d(
  ...
  contour = F
                             ...
                             values = color.scaling) + xlab(heatmap.label.norm_pctle) + ylab(heatmap.label.nf.order)
...
ggplot(m.newsfeed.df.all, aes(factor(holder), nf.order)) + 
  stat_density(aes(fill = after_stat(density)), geom = "raster", position = "identity") +
  facet_grid(.~group, scales = "free_y") +
  scale_y_reverse() + 
  scale_fill_gradientn(colors = brewer.pal(n = 5, name = "Blues"), values = color.scaling) + 
  theme(axis.title.x =element_blank(),
        axis.text.x =element_blank(),
        axis.ticks.x = element_blank(), legend.position="none") + 
  ylab(heatmap.label.nf.order) 
...
heatmap.label.pymk.order <- "Actual PYMK Ranking"
#heatmap.label.complete_time_rank <- "Chronological Ranking"
#heatmap.label.norm_pctle <- "User's Self-Reported Preferences"
...
ggplot(m.pymk.df.all, aes(x=norm.pctle, pymk.order)) + stat_density_2d(
  ...
  contour = F
                             ...
                             values = color.scaling) + ylab(heatmap.label.pymk.order)
...
topleft <- ggplot(l.dat, aes(x=preference, y=mean.rating)) +
  ...
  geom_line(position=pd) + labs(title = "Newsfeed Ranking by Preference") + 
...
topright <- ggplot(l.dat, aes(x=preference, y=prob5)) +
  ...
  labs(title = "Newsfeed Top 5 by Preference") + 
  ylab("Pr(Top 5)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
botleft <- ggplot(l.dat, aes(x=preference, y=prob10)) +
  ...
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + 
  labs(title = "Newsfeed Top 10 by Preference") + 
  ylab("Pr(Top 10)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
botright <- ggplot(l.dat, aes(x=preference, y=prob15)) +
  ...
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + 
  labs(title = "Newsfeed Top 15 by Preference") + 
  ylab("Pr(Top 15)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
for (boot.n in 1:1000) {
  dat <- slice_sample(m.newsfeed.df.all, n =nrow(m.newsfeed.df.all), replace = T)%>% group_by(nf.order.round, race.in.group) %>% dplyr::summarise(n= n()) %>%
  pivot_wider(names_from = race.in.group, values_from = n)
...
dat.se <- holder %>% group_by(nf.order.round) %>% dplyr::summarise(sd= sd(excess))
...
ggplot(dat, aes(x = nf.order.round, y = excess)) + geom_bar(stat = "identity") +
  ...
  ylab("Excess Ingroup Mass %") +
  xlab("Newsfeed rank sextile (Groups of 10 posts)")  +
  scale_x_discrete(name ="Actual Newsfeed Ranking Group", 
                   breaks = c("1","2","3","4","5","0"),
                    labels=c("11-20","21-30", "31-40", "41-50", "51-60", "Top 10")) + ylim(c(-.19,.19)) +
  coord_flip()
...
topleft <- ggplot(l.dat, aes(x=preference, y=mean.rating, colour=race.in.group, group=race.in.group)) +
  ...
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "Newsfeed Ranking by Preference") + 
  ylab("Mean number of slots \nabove median rank") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
common.x.lab <- "User self-reported preference \nquartile (4=most preferred)"
...
ggplot(l.dat, aes(x=factor(chron_group), y=mean.rating, colour=race.in.group, group=race.in.group)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci, colour=race.in.group), width=.1, position=pd) + 
  geom_point(aes(shape = race.in.group), position=pd, size=3) +
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "Newsfeed Ranking by Age of Post") + 
  ylab("Mean number of slots \nabove median rank") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
ggplot(l.dat %>% filter(norm.quartile == 1 & race.in.group == F), aes(x=norm.quartile, y=mean.rating, colour=race.in.group, group=race.in.group)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci, colour=race.in.group), width=.1, position=pd) + 
  geom_point(aes(shape = race.in.group), position=pd, size=3) +
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "Newsfeed Ranking by Preference") + xlim(c(.9,4.1)) +
  ylab("Mean number of slots \nabove median rank") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
ggplot(l.dat %>% filter(norm.quartile == 1), aes(x=norm.quartile, y=mean.rating, colour=race.in.group, group=race.in.group)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci, colour=race.in.group), width=.1, position=pd) + 
  geom_point(aes(shape = race.in.group), position=pd, size=3) +
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "Newsfeed Ranking by Preference") + ylim(c(-2, 6)) + xlim(c(.9,4.1)) +
  ylab("Mean number of slots \nabove median rank") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
for (b in 1:1000) {
  ...
  m.recent.df <- read.csv("/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Temp/Clean US Data Recent.csv")
  m.recent.df <- sample_n(m.recent.df, nrow(m.recent.df), replace = T) %>% filter(human == 1)
  ...
  m.newsfeed.df.all.b <- sample_n(m.newsfeed.df.all, nrow(m.newsfeed.df.all), replace = T)
  
  
  base.rate <- mean((m.newsfeed.df.all.b %>%
    filter(source %in% c("CDR 2", "CDR 4")) %>% 
    dplyr::select(race.in.group))$race.in.group, na.rm = T)
  
  base.rate.all <- mean((m.newsfeed.df.all.b %>%
    dplyr::select(race.in.group))$race.in.group, na.rm = T)
  
  nf.5 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(new.rank <= 5) %>% 
  ...
  nf.10 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(new.rank <= 10) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 1)
  nf.10$source <- "Actual Newsfeed Ranking"
  
  nf.20 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(new.rank <= 20) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
  ...
  pref.5 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(norm.pctle >= (1-(5/60))) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 0)
  pref.5$source <- "Preference Ranking"
  
  pref.10 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(norm.pctle >= (1-(10/60))) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 1)
  pref.10$source <- "Preference Ranking"
  
  pref.20 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
  ...
  pref.5.all <- m.newsfeed.df.all.b %>%
    filter(norm.pctle >= (1-(5/60))) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 0)
  pref.5.all$source <- "Preference Ranking (All)"
  
  pref.10.all <- m.newsfeed.df.all.b %>%
    filter(norm.pctle >= (1-(10/60))) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 1)
  pref.10.all$source <- "Preference Ranking (All)"
  
  pref.20.all <- m.newsfeed.df.all.b %>%
    filter(norm.pctle >= (1-(20/60))) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 2)
  pref.20.all$source <- "Preference Ranking (All)"
  
  chron.5 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(time_rank <= 5) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 0)
  chron.5$source <- "Chronological Ranking"
  
  chron.10 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(time_rank <= 10) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 1)
  chron.10$source <- "Chronological Ranking"
  
  chron.20 <- m.newsfeed.df.all.b %>% filter(source %in% c("CDR 2", "CDR 4")) %>%
    filter(time_rank <= 20) %>% 
    dplyr::summarise(prob.same.race = mean(race.in.group, na.rm = T), sd = sd(race.in.group, na.rm = T), n= n()) %>%
    dplyr::mutate(group = 2)
  chron.20$source <- "Chronological Ranking"
  
  l.dat <- rbind(nf.5, nf.10, nf.20, recent.summ.i)
...
holder <- rbind(holder, l.dat)

}

l.dat.se <- holder %>% group_by(group, source) %>% dplyr::summarise(sd.rate = sd(prob.same.race.rate), sd.diff = sd(prob.same.race.diff))
...
l.dat <- merge(l.dat, l.dat.se)
l.dat$ci.diff <- l.dat$sd.diff*1.96
l.dat$ci.rate <- l.dat$sd.rate*1.96
...
ggplot(l.dat, aes(x = factor(group), y=prob.same.race.rate, fill = source)) + 
    geom_bar(stat="identity", position=pd) +
  geom_errorbar(aes(ymin=prob.same.race.rate-ci.rate, ymax=prob.same.race.rate+ci.rate), width=.1, position=pd) + labs(title = "Ingroup preferences in interactions") + ylab("Share of posts that are in-group \ndivided by base rate") + 
  scale_x_discrete(name ="Ranking Group", 
                   breaks = c("1","2","0", "-1", "-2", "-3"),
                    labels=c("Top 10", "Top 20", "Top 5", "Comments", "React", "10 most recent")) + 
  scale_fill_manual(values=rev(brewer.pal(n = 6, name = "Greys")[c(5,2)])) 
...
ggplot(l.dat, aes(x = factor(group), y=prob.same.race.diff, fill = source)) + 
    geom_bar(stat="identity", position=pd) +
  geom_errorbar(aes(ymin=prob.same.race.diff-ci.diff, ymax=prob.same.race.diff+ci.diff), width=.1, position=pd) + labs(title = "Ingroup preferences in interactions") + ylab("Share of posts that are in-group \nminus base rate") + 
  scale_x_discrete(name ="Ranking Group", 
                   breaks = c("1","2","0", "-1"),
...
ggplot(l.dat, aes(x=pymk.order.round, y=prob.same.race)) + 
    geom_errorbar(aes(ymin=prob.same.race-ci, ymax=prob.same.race+ci), colour="black", width=.1, position=pd) +
    geom_line(position=pd) +
    geom_point(position=pd, size=3) + geom_hline(yintercept = mean(m.pymk.df.all$race.in.group), color="blue", linetype="dashed", size=1) + xlab("FB PYMK Ranking (increments of 10)") + ylab("Rate of same-race recommendations")  + ylim(c(0.50,0.65)) + labs(title = "PYMK preference for user's race ", subtitle = "Same-race recommendations are not sorted closer to the top")  + 
...
topleft <- ggplot(l.dat, aes(x=familiarity, y=mean.rating)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci), width=.1, position=pd) + 
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + labs(title = "PYMK Ranking by Preference") + 
  ylab("Mean number of slots \nabove median rank") + 
  xlab(common.x.lab)
...
topright <- ggplot(l.dat, aes(x=familiarity, y=prob5)) +
  ...
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + 
  labs(title = "PYMK Top 5 by Preference") + 
  ylab("Pr(Top 5)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
botleft <- ggplot(l.dat, aes(x=familiarity, y=prob10)) +
  ...
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + 
  labs(title = "PYMK Top 10 by Preference") + 
  ylab("Pr(Top 10)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
botright <- ggplot(l.dat, aes(x=familiarity, y=prob15)) +
  ...
  geom_point(position=pd, size=3) +
  geom_line(position=pd) + 
  labs(title = "PYMK Top 15 by Preference") + 
  ylab("Pr(Top 15)") + 
  xlab(common.x.lab) +
  scale_colour_manual(values=outgroup.ingroup.colors, 
                       name="Poster Identity",
...
ggplot(l.dat, aes(x=norm.quartile, y=mean.rating, colour=race.in.group, group=race.in.group)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci, colour=race.in.group), width=.1, position=pd) + 
  geom_point(aes(shape = race.in.group), position=pd, size=3) +
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "PYMK Ranking by Familiarity") + ylim(c(-2, 6)) + xlim(c(.9,4.1)) +
  ylab("Mean number of slots \nabove median rank") + 
...
ggplot(l.dat, aes(x=friend.group, y=mean.rating, colour=race.in.group, group=race.in.group)) +
  geom_errorbar(aes(ymin=mean.rating-ci, ymax=mean.rating+ci, colour=race.in.group), width=.1, position=pd) + 
  geom_point(aes(shape = race.in.group), position=pd, size=3) +
  geom_line( aes(linetype=race.in.group, color=race.in.group) , position=pd) + labs(title = "PYMK Ranking by Familiarity") + ylim(c(-2, 6)) +
  ylab("Mean number of slots \nabove median rank") + 
...
ggplot(dat, aes(x = pymk.order.round, y = excess)) + geom_bar(stat = "identity") +
  ...
  scale_x_discrete(name ="PYMK Ranking Group", 
                   breaks = c("1","2","3","4","5","0"),
                    labels=c("11-20","21-30", "31-40", "41-50", "51-60", "Top 10")) + ylim(c(-.17,.17)) +
  coord_flip() 

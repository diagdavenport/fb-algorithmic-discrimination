...
# 
# dev.copy(jpeg,'Output/Graphs/Audit/Sankey flows/US NF chronology to actual by norm preference.jpg')
# dev.off()
# 
...
# 
# dev.copy(jpeg,'Output/Graphs/Audit/Sankey flows/US NF norm preference to actual by ingroup.jpg')
# dev.off()
# 
...
#   scale_fill_manual(values=c("red", "blue", "snow"))
# 
# dev.copy(jpeg,'Output/Graphs/Audit/Sankey flows/US NF chronology to actual by ingroup.jpg')
# dev.off()
...
ggplot(m.newsfeed.df.all, aes(x=norm.sixty, nf.order)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)),
  contour = F
) + scale_fill_viridis_c(option = "D") + scale_y_reverse()

ggplot(m.newsfeed.df.all, aes(x=norm.sixty, nf.order)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)),
  contour = F
) + scale_fill_viridis_c(option = "C") + scale_y_reverse()
...
ggplot(m.newsfeed.df.all, aes(x=norm.pctle, nf.order)) + stat_density_2d(
  ...
  aes(fill = after_stat(density)),
  contour = F
) + scale_fill_viridis_c(option = "B") + scale_y_reverse()

ggplot(m.newsfeed.df.all, aes(x=norm.pctle, nf.order)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)),
  contour = F
) + scale_fill_viridis_c(option = "A") + scale_y_reverse()
...
ggplot(m.newsfeed.df.all, aes(x=norm.pctle, nf.order)) + stat_density_2d(
  ...
  scale_fill_gradient(
                      low = "#FFFFFF",
                      high = "#012345", breaks = c(0.19, 0.02))

ggplot(m.newsfeed.df.all, aes(x=norm.pctle, nf.order)) + stat_density_2d(
  geom = "raster",
  aes(fill = after_stat(density)),
  contour = F
) + scale_y_reverse() + scale_fill_gradientn(colors = brewer.pal(n = 5, name = "Reds"),
                             values = c(0, 0.8, 0.9, 0.95, 1))
...
l.dat <- rbind(nf.5, nf.10, nf.20, recent.summ.r, recent.summ.c, recent.summ.i)
...
l.dat$prob.same.race.diff <- (l.dat$prob.same.race-base.rate)

...
df.out <- data.frame(x = rnorm(800, mean = 0), identity = "Outgroup")
inner <- data.frame(x = rnorm(800, mean = 1), identity = "Ingroup")

complete <- rbind(df.out, inner)

mu <- ddply(complete, "identity", summarise, grp.mean=mean(x, na.rm = T))

ggplot(complete, aes(x=x, color=identity)) +
  geom_density() + geom_vline(data=mu, aes(xintercept=grp.mean, color=identity),
             linetype="dashed") + xlab("Hypothetical Revealed Preference Index") +
  scale_colour_manual(values=c("#CC79A7", "#009E73"), 
                       name="Poster Identity",
                       breaks=c("Outgroup", "Ingroup"),
...
m.newsfeed.df.all <- read.csv("/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Data/Clean US Data NF.csv")

m.pymk.df.all <- read.csv("/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Data/Clean US Data PYMK.csv")

m.qual.df.all <- read.csv("/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Data/Clean qualtrics data.csv")
...
ggplot(newsfeed.with.misranks, aes(x=norm.pctle, y=chron.misrank)) + 

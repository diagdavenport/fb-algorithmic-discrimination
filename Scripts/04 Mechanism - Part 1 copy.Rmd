...
output.dta <- read.csv("/Users/diagdavenport/Downloads/Output-2022-02-22.csv")

user.dta <- read.csv("/Users/diagdavenport/Downloads/User-2022-02-22.csv")
...
sql.data$good.data <- sql.data$date >= ymd("2022-02-20") # Full run 1
...
l.dat <- sql.data %>% group_by(timed, order_no.ceiling) %>% dplyr::summarise(mean = mean(in.group, na.rm = T), sd = sd(in.group, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(order_no.ceiling))) + ylab("% same race") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + xlab("Rushed?") 




l.dat <- sql.data %>% group_by(timed, order_no.ceiling) %>% dplyr::summarise(mean = mean(same.gender, na.rm = T), sd = sd(same.gender, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(order_no.ceiling))) + ylab("% same gender") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + xlab("Rushed?") 
...
mdl.fast.g <- lm(order_no ~ (in.group.lax
                 +genre.match), data = sql.data %>% filter(timed == 1))
mdl.fast.g1 <- lm(order_no ~ (in.group.lax
                 +genre.match), data = sql.data %>% filter(timed == 0))

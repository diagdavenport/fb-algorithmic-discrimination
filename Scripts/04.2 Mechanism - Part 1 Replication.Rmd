---
title: "Process mech part 1 - vary time, white men, demo first (slow validation)"
author: "ddd"
date: "5/25/2021"
output: pdf_document
...
output.dta <- read.csv("/Users/diagdavenport/Downloads/Output-2021-12-27.csv")

user.dta <- read.csv("/Users/diagdavenport/Downloads/User-2021-12-27.csv")
...
sql.data$hr <- hour(sql.data$timestamp)

sql.data$mth <- month(sql.data$timestamp)

sql.data$good.data <- sql.data$date >= ymd("2021-12-23") 
...
#sql.data <- sql.data %>% filter(hr < 20)
...
#sql.data$race.simple <- ifelse(sql.data$race.cat.2 != "BLANK", "Multi", sql.data$race.simple)

#sql.data <- sql.data %>% filter(race.simple != "Other")
  
sql.data$in.group <- sql.data$race.simple == sql.data$rec_race

# sql.data$in.group <- ifelse(sql.data$race.simple == "Multi" & sql.data$rec_race != "White",
#                             T,
#                             sql.data$in.group)
...
sql.data$rec_race <- factor(sql.data$rec_race, levels = c("White", "Asian", "Black", "Hispanic", "Other"))
...
sql.data$order_group <- floor(sql.data$order_no/10)

sql.data <- sql.data %>% filter(race.simple == "White" & user_gender == "Male")


sql.data$clean_user_genre <- ""

sql.data$clean_user_genre <- with(sql.data,
                 ifelse(user_genre %in% c("Historical", "Horror", "Other"),
                        "HHO",
                        as.character(user_genre)))
...
dev.copy(jpeg,'Output/Graphs/Experiments/Movie Mechanism/Replication/Lax ingroup percent of clicks.jpg')
dev.off()
...
dev.copy(jpeg,'Output/Graphs/Experiments/Movie Mechanism/Replication/Lax ingroup click rate.jpg')
dev.off()

l.dat <- sql.data %>% group_by(timed, in.group.lax, genre.match) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())
...
ggplot(l.dat, aes(x=factor(timed), y=mean, fill = in.group.lax)) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  ...
  scale_colour_manual(values=outgroup.ingroup.colors, 
   ...
   scale_fill_manual(values=outgroup.ingroup.colors,
                     name="Poster Identity",
                       breaks=c("FALSE", "TRUE"),
                       labels=c("Outgroup", "Ingroup")) + scale_x_discrete(labels=c("0" = "No", "1" = "Yes")) + facet_grid(.~genre.match)
...
stargazer(lm(clicked ~ in.group.lax*deliberate, data = sql.data),
          title = "Experiment 2, Human Bias",
          out = "Output/Tex/Main results/Experiment 2, Human Bias.tex")
...
l.dat <- sql.data %>% group_by(timed, group.combo, genre.match) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(group.combo))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + xlab("Rushed?") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  + labs(fill = "Same gender - Same race") + facet_wrap(.~genre.match)


sql.data$order_no.group.6 <- ceiling(sql.data$order_no/15)

l.dat <- sql.data %>% group_by(timed, user_id) %>% dplyr::summarise(options.considered= max(order_no))
ggplot(l.dat, aes(x=options.considered, fill=factor(timed))) +
    geom_density(alpha=0.8)

l.dat <- sql.data %>% filter(readmore_count > 0) %>% group_by(timed, order_no.group.6) %>% dplyr::summarise(count= n()) %>% group_by(timed) %>% mutate(percent = count/sum(count), T = sum(count))

l.dat$sd <- l.dat$percent * (1-l.dat$percent)
l.dat$ci <- (l.dat$sd/sqrt(l.dat$T))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x= factor(order_no.group.6), y=percent, fill = factor(timed))) + ylab("% of reads") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=percent-ci, ymax=percent+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))


l.dat <- sql.data %>% filter(clicked == 1) %>% group_by(timed, order_no.group.6) %>% dplyr::summarise(count= n()) %>% group_by(timed) %>% mutate(percent = count/sum(count), T = sum(count))

l.dat$sd <- l.dat$percent * (1-l.dat$percent)
l.dat$ci <- (l.dat$sd/sqrt(l.dat$T))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x= factor(order_no.group.6), y=percent, fill = factor(timed))) + ylab("% of clicks") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=percent-ci, ymax=percent+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))


l.dat <- sql.data %>% filter(clicked == 1) %>% group_by(timed, order_no.group.6, genre.match) %>% dplyr::summarise(count= n()) %>% dplyr::group_by(timed) %>% mutate(percent = count/sum(count), T = sum(count))

l.dat$sd <- l.dat$percent * (1-l.dat$percent)
l.dat$ci <- (l.dat$sd/sqrt(l.dat$T))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x= factor(order_no.group.6), y=percent, fill = factor(genre.match))) + ylab("% of clicks") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=percent-ci, ymax=percent+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(.~timed)



l.dat <- sql.data %>% group_by(timed, order_no.group.6) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(order_no.group.6), y=mean, fill = factor(timed))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + xlab("Rushed?") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  + labs(fill = "In-group?") + facet_wrap(.~genre.match)

l.dat <- sql.data %>% group_by(timed, in.group.lax, order_no.group.6, genre.match) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(genre.match~order_no.group.6, scales = "free")



l.dat <- sql.data %>% group_by(timed, in.group.lax, genre.match, rating.x) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(genre.match~rating.x)



sql.data$older.age <- median(sql.data$user_age) >= sql.data$user_age

l.dat <- sql.data %>% group_by(timed, in.group.lax, older.age, genre.match) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(timed), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(genre.match~factor(older.age))




l.dat <- sql.data %>% filter(timed == 1) %>% group_by(in.group.lax, genre.match, clean_edu) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(clean_edu), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + facet_grid(genre.match~.)



l.dat <- sql.data %>% filter(timed == 1) %>% group_by(in.group.lax, clean_user_genre) %>% dplyr::summarise(mean = mean(clicked, na.rm = T), sd = sd(clicked, na.rm = T), n= n())

l.dat$ci <- (l.dat$sd/sqrt(l.dat$n))*1.96
pd <- position_dodge(0.01)

ggplot(l.dat, aes(x=factor(clean_user_genre), y=mean, fill = factor(in.group.lax))) + ylab("pr(click)") + geom_bar(stat="identity", position=position_dodge(), width = .8)  +
  geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.05, position=position_dodge(.8)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
```{r, person heterogeneity}

ggplot(sql.data %>% filter(clicked == 1), aes(order_no, colour = factor(timed))) +
  stat_ecdf() + facet_grid(.~genre.match)

l.dat <- sql.data %>% filter(clicked == 1) %>% group_by(user_id, timed, genre.match) %>% dplyr::summarise(in.group.prop.clicks = mean(in.group.lax, na.rm = T)) 

ggplot(l.dat, aes(in.group.prop.clicks, colour = factor(timed))) +
  stat_ecdf() + facet_grid(.~genre.match)


l.dat <- sql.data %>% group_by(user_id, timed, genre.match) %>% dplyr::summarise(in.group.observed.rate = mean(in.group.lax, na.rm = T)) 

ggplot(l.dat, aes(in.group.observed.rate, colour = factor(timed))) +
  stat_ecdf() + facet_grid(.~genre.match)


ggplot(l.dat, aes(x=in.group.observed.rate, fill=factor(timed))) +
    geom_density(alpha=0.8) 

l.dat <- sql.data %>% filter(in.group.lax == 1) %>% group_by(user_id, timed, genre.match) %>% dplyr::summarise(click.rate = mean(clicked, na.rm = T))

ggplot(l.dat, aes(click.rate, colour = factor(timed))) +
  stat_ecdf() + facet_grid(.~genre.match)


l.dat <- sql.data  %>% filter(clicked == 1) %>% group_by(user_id, timed, genre.match) %>% dplyr::summarise(total.in.clicks = sum(in.group.lax, na.rm = T))

ggplot(l.dat, aes(total.in.clicks, colour = factor(timed))) +
  stat_ecdf() + facet_grid(.~genre.match)


left <- sql.data %>% group_by(user_id, timed) %>% dplyr::summarise(in.group.observed = mean(in.group.lax, na.rm = T),
                                                                   total.seen = max(order_no))
right <- sql.data %>% filter(clicked == 1) %>% group_by(user_id, timed) %>% dplyr::summarise(in.group.selected = mean(in.group.lax, na.rm = T))

l.dat <- merge(left,right)

l.dat$overrepresented.ingroup <- with(l.dat, (in.group.selected - in.group.observed)/in.group.observed)
l.dat$overrepresented.ingroup.level <- with(l.dat, (in.group.selected - in.group.observed))

ggplot(l.dat, aes(overrepresented.ingroup, colour = factor(timed))) +
  stat_ecdf()

ggplot(l.dat, aes(x=total.seen, y=overrepresented.ingroup, shape=factor(timed), color=factor(timed))) +
  geom_point() +
  geom_smooth(method=lm, se=T, linetype="dashed")


ggplot(l.dat, aes(x=overrepresented.ingroup, fill=factor(timed))) +
    geom_density(alpha=0.8) + geom_vline(xintercept = mean(l.dat[l.dat$timed==0,"overrepresented.ingroup"]), linetype = "dotted") + geom_vline(xintercept = mean(l.dat[l.dat$timed==1,"overrepresented.ingroup"]), linetype = "dashed")


ggplot(l.dat, aes(x=overrepresented.ingroup.level, fill=factor(timed))) +
    geom_density(alpha=0.8) + geom_vline(xintercept = mean(l.dat[l.dat$timed==0,"overrepresented.ingroup.level"]), linetype = "dotted") + geom_vline(xintercept = mean(l.dat[l.dat$timed==1,"overrepresented.ingroup.level"]), linetype = "dashed")


summary(lm(in.group.selected ~ timed*in.group.observed, data = l.dat))

summary(lm(in.group.observed ~ timed, data = l.dat))

l.dat <- sql.data %>% group_by(user_id, timed, order_no.group.6) %>% dplyr::summarise(in.group.rate = mean(in.group.lax, na.rm = T))

ggplot(l.dat, aes(x=in.group.rate, fill=factor(timed))) +
    geom_density(alpha=0.8)  +facet_wrap(.~order_no.group.6)

l.dat <- sql.data %>% filter(clicked == 1) %>% group_by(user_id, timed, order_no.group.6) %>% dplyr::summarise(in.group.rate = mean(in.group.lax, na.rm = T))
...
stargazer(mdl.race, mdl.gender, mdl.lax, mdl.strict, omit = c("movie_title", "user_id", "rating", "order_no"),
          omit.stat = c("f",    "ser")
...
d.test <- test.data %>%
      select(!!!enquos(vars.of.interest), user_id, in.group.lax, clean_user_genre)
...
write.csv(d.test, file = "Temp/Part 1 replication - Full slow algortithm data.csv")
...
#file = "/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Scripts/05 - deep model.pmml"
#r2pmml(tree.mdl.fast, file)

#file = "/Users/diagdavenport/Desktop/Synced Research/Ludwig/FB/Lockdown/Scripts/05 - shallow model.pmml"
#r2pmml(tree.mdl.fast, file, compact = T)

# rpart.plot(tree.mdl.fast)

d.test$prediction.fast <- predict(tree.mdl.fast, d.test)

d.test <- d.test %>%
    group_by(user_id) %>% 
    mutate(rank.fast = rank(-prediction.fast, ties.method = "random"))

write.csv(d.test, file = "Temp/Part 1 replication - Full fast algortithm data.csv")
...
dev.copy(jpeg,'Output/Graphs/Experiments/Movie Mechanism/Replication/Lax ingroup mean algorithm ranking.jpg')
dev.off()
...
dev.copy(jpeg,'Output/Graphs/Experiments/Movie Mechanism/Replication/Lax ingroup top 6 algorithm.jpg')
dev.off()
...
first.mdl.c <- lm(rank ~ in.group.lax*psych + genre.match, data = full.data)
sec.mdl.c <- lm(top ~ in.group.lax*psych + genre.match, data = full.data)
  
stargazer(first.mdl, sec.mdl, first.mdl.c, sec.mdl.c,
          omit.stat = c("f",    "ser", "rsq"),
          title = "Experiment 2, Algorithmic Bias",
          out = "Output/Tex/Main results/Experiment 2 Algorithmic Bias.tex")

```

```{r, results = 'asis'}

mdl <- lm(rank.fast ~ in.group.lax*(user_age + clean_edu + user_frequency + clean_user_genre + genre.match + rating.y + rec_gender + rec_race), data = full.fast.data)

mdl.top <- lm(top ~ in.group.lax*(user_age + clean_edu + user_frequency + clean_user_genre + genre.match + rating.y + rec_gender + rec_race), data = full.fast.data)

stargazer(mdl, mdl.top, omit.stat = c("f",    "ser", "rsq"), keep = "in.group.lax")

```

```{r}

tree.mdl.fast$importance
